...
def get_word_score(word, n):
    ...
    for char in word_lower:
        if char != '*':
            fact1 += SCRABBLE_LETTER_VALUES[char]
    fact2 = (7*len(word)) - (3*(n - len(word)))
    if fact2 <1:
        fact2 = 1
    return fact1 * fact2
...
def is_valid_word(word, hand, word_list):
    ...
    # 1st test: word is entirely composed of letters in the hand
    hand_copy = hand.copy()
    for char in word.lower():
    ...
    # 2nd test: word is in the word_list:
    if word.lower() in word_list:
        return True
    elif '*' in word:
        for v in VOWELS:
            new_word = word.lower().replace('*', v)
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while len(hand) > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(user_input, hand, word_list):
                ...
                total += score;
                print('"' + user_input + '"' + " earned", score, "points. Total:", total, "points")
    ...
    if len(hand) > 0:
        ...
        print('Ran out of letters. Total score for this hand:', total, 'points')
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while i < num_hands:
        ...
        display_hand(hand)
        while True:
            is_subs = input('Would you like to substitute a letter (yes/no)? ' ).lower()
            if is_subs == "yes":
        ...
        print()
        grand_total += play_hand(hand, word_list)
        i += 1
        if i != (num_hands):
            while True:
                is_replay = input("Would you like to replay the hand (yes/no)? ").lower()
                if is_replay == "yes":
                    break
                elif is_replay == "no":
                    hand = deal_hand(HAND_SIZE)
