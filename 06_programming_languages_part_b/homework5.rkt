  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([funexp (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? funexp)
               (let ([cloenv (closure-env funexp)]
                     [clofun (closure-fun funexp)]
                     [vactual (eval-under-env actual env)])
                 (let* ([funname (fun-nameopt clofun)]
                        [funformal (fun-formal clofun)]
                        [funbody (fun-body clofun)]
                        [funenv (cons (cons funformal vactual) cloenv)])
                   (if funname
                       (eval-under-env funbody
                                       (cons (cons funname funexp) funenv))
                       (eval-under-env funbody
                                       funenv))))
               (error "MPUL call applied to non-closure")))]
        [(apair? e)
        ...
        [(fst? e)
         (let ([v (eval-under-env (fst-e e) env)])
           (if (apair? v)
               (apair-e1 v)
               (error "MPUL fst applied to non-apair")))]
        [(snd? e)
         (let ([v (eval-under-env (snd-e e) env)])
           (if (apair? v)
               (apair-e2 v)
               (error "MPUL snd applied to non-apair")))]
        [(isaunit? e)
